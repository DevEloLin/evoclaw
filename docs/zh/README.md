...
Phase 7（多渠道）和 Phase 8（深度加固）显式标记为后续工作 —— Telegram / Slack 插件依赖外部服务 token，Local Dashboard 需要 Tauri 重壳，内核级 sandbox 与负载测试也不是个人开发者使用 runtime 的阻塞项。Phases 1–6 全部在 v0.1.6 中交付。
