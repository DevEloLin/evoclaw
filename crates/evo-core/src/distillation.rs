...
pub fn parse_distilled_skill(model_text: &str, task_id: &str) -> Result<Skill, String> {
    ...
    let id = json.get("id").and_then(|v| v.as_str()).unwrap_or("skill-untitled").to_string();
    let name = json.get("name").and_then(|v| v.as_str()).unwrap_or(&id).to_string();
    let kind: SkillKind = json.get("type")
    ...
    if let Some(desc) = json.get("description").and_then(|v| v.as_str()) { sk.description = desc.to_string(); }
    if let Some(arr) = json.get("triggers").and_then(|v| v.as_array()) {
        sk.triggers = arr.iter().filter_map(|v| v.as_str().map(String::from)).take(6).collect();
    }
    if let Some(arr) = json.get("preconditions").and_then(|v| v.as_array()) {
        sk.preconditions = arr.iter().filter_map(|v| v.as_str().map(String::from)).take(5).collect();
    }
    if let Some(steps) = json.get_mut("steps").map(std::mem::take) {
        if let Ok(parsed) = serde_json::from_value(steps) { sk.steps = parsed; }
    }
    if let Some(v) = json.get("verification").and_then(|v| v.as_str()) { sk.verification = v.to_string(); }
    if let Some(arr) = json.get("success_criteria").and_then(|v| v.as_array()) {
        sk.success_criteria = arr.iter().filter_map(|v| v.as_str().map(String::from)).collect();
    }
    if let Some(arr) = json.get("failure_patterns").and_then(|v| v.as_array()) {
        sk.failure_patterns = arr.iter()
...
pub fn skill_from_reflection_quick(
    ...
    let id = format!(
        "skill-{}",
        task_id.trim_start_matches("task-").chars().take(12).collect::<String>()
    );
    ...
    sk.failure_patterns = reflection.failure_patterns.iter().map(|p| crate::skill::FailurePattern {
        pattern: p.clone(),
...
mod tests {
    ...
    fn quick_skill_uses_reflection_summary_and_goal() {
        let sk = skill_from_reflection_quick(&refl(), "task-20260502T120000.000", "diagnose ssh hang");
