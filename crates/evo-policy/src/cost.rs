...
pub enum BudgetLevel { Task, Day, Month }
...
impl Default for BudgetCfg {
    fn default() -> Self { Self { per_task_usd: 0.50, per_day_soft_usd: 5.0, per_day_hard_usd: 20.0, per_month_usd: 100.0 } }
}
...
impl CostEngine {
    pub fn at(log_path: impl Into<PathBuf>, cfg: BudgetCfg) -> Self {
        Self { log_path: log_path.into(), cfg }
    }

    pub fn cfg(&self) -> &BudgetCfg { &self.cfg }
    ...
    pub async fn record(&self, event: &CostEvent) -> std::io::Result<()> {
        ...
        let mut existing = tokio::fs::read_to_string(&self.log_path).await.unwrap_or_default();
    ...
    pub async fn check_for_task(&self, task_id: &str) -> std::io::Result<BudgetCheck> {
        ...
        if s.month_usd > self.cfg.per_month_usd { return Ok(BudgetCheck::HardStop(BudgetLevel::Month)); }
        if s.day_usd > self.cfg.per_day_hard_usd { return Ok(BudgetCheck::HardStop(BudgetLevel::Day)); }
        if s.task_usd > self.cfg.per_task_usd { return Ok(BudgetCheck::HardStop(BudgetLevel::Task)); }
        if s.day_usd > self.cfg.per_day_soft_usd { return Ok(BudgetCheck::SoftWarn(BudgetLevel::Day)); }
        Ok(BudgetCheck::Ok)
    }
    ...
    pub async fn summary_for_task(&self, task_id: &str) -> std::io::Result<CostSummary> {
        ...
        for ev in self.read_events().await? {
            if ev.task_id == task_id { s.task_usd += ev.usd; }
            if ev.ts >= day_start    { s.day_usd  += ev.usd; }
            if ev.ts >= month_start  { s.month_usd += ev.usd; }
            s.events += 1;
            total_input += ev.input_tokens;
            total_cached += ev.cached_tokens;
        }
        s.avg_cache_hit_rate = if total_input == 0 { 0.0 } else { total_cached as f64 / total_input as f64 };
        Ok(s)
    }
    ...
    pub async fn read_events(&self) -> std::io::Result<Vec<CostEvent>> {
        ...
        for line in text.lines() {
            ...
            if let Ok(ev) = serde_json::from_str::<CostEvent>(line) { out.push(ev); }
        }
        Ok(out)
    }
}
...
mod tests {
    ...
    fn ev(usd: f64, task: &str) -> CostEvent {
        CostEvent {
            ts: Utc::now(), task_id: task.into(), model: "m".into(),
            input_tokens: 1000, cached_tokens: 600, output_tokens: 50, usd,
        }
    }
