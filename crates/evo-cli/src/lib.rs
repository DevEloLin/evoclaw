...
use evo_providers::{AcpProvider, AnthropicProvider, CopilotProvider, OpenAiCompatProvider, Provider};
...
enum SecretCmd {
    ...
    Add { name: String, #[arg(long)] stdin: bool, value: Option<String> },
...
#[derive(Subcommand, Debug)]
enum DoctorCmd { Tokens, Closure }

#[derive(Subcommand, Debug)]
enum SkillCmd { List, Show { id: String }, Tree }
...
fn skills_dir() -> Result<PathBuf> { Ok(evoclaw_dir()?.join("skills")) }
fn memory_dir() -> Result<PathBuf> { Ok(evoclaw_dir()?.join("memory")) }
fn cost_log_path() -> Result<PathBuf> { Ok(evoclaw_dir()?.join("cost.jsonl")) }
fn vault_path() -> Result<PathBuf> { Ok(default_vault_path(&evoclaw_dir()?)) }

#[derive(Debug, Clone, Serialize, Deserialize)]
struct Config { model: ModelCfg, budget: ConfigBudget, security: SecurityCfg }
#[derive(Debug, Clone, Serialize, Deserialize)]
struct ModelCfg {
...
#[derive(Debug, Clone, Serialize, Deserialize)]
struct ConfigBudget { per_task_usd: f64, per_day_usd: f64, per_month_usd: f64 }
#[derive(Debug, Clone, Serialize, Deserialize)]
struct SecurityCfg { default_permission: String, high_risk_intercept: bool }
...
fn evoclaw_dir() -> Result<PathBuf> { Ok(home()?.join(".evoclaw")) }
fn config_path() -> Result<PathBuf> { Ok(evoclaw_dir()?.join("config.toml")) }
fn workspace_dir() -> Result<PathBuf> { Ok(evoclaw_dir()?.join("workspace")) }
fn logs_dir() -> Result<PathBuf> { Ok(evoclaw_dir()?.join("logs")) }

async fn ensure_layout() -> Result<()> {
    for sub in ["workspace", "logs", "skills", "browser_profiles", "secrets", "plugins", "cache"] {
        tokio::fs::create_dir_all(evoclaw_dir()?.join(sub)).await
...
async fn load_config() -> Result<Config> {
    ...
    let text = tokio::fs::read_to_string(&p).await
        .wrap_err_with(|| format!("read config at {}; run `evoclaw onboard` first", p.display()))?;
...
pub async fn entry() -> Result<()> {
    ...
    match cli.cmd {
        ...
        Some(Cmd::Agent(a))                 => match a {
            ...
            AgentCmd::Catalog      => { agent_catalog(); Ok(()) }
        ...
        Some(Cmd::Mcp(m))                   => match m {
            ...
            McpSubCmd::Catalog      => { mcp_catalog(); Ok(()) }
...
async fn interactive() -> Result<()> {
    ...
    loop {
        ...
        if let Some(rest) = line.strip_prefix('/') {
            if handle_slash(rest).await? { return Ok(()); }
            continue;
        }
...
async fn print_banner(cfg: &Config) {
    let provider_id = cfg.model.provider.clone().unwrap_or_else(|| "deepseek".into());
    ...
    let home = evoclaw_dir().map(|p| p.display().to_string()).unwrap_or_default();
    ...
    println!("{cyan_b}   ║      ╚══════╝  ╚═══╝   ╚═════╝  ╚═════╝╚══════╝╚═╝  ╚═╝╚═╝    ║{reset}");
    println!("{cyan_b}   ║                                                               ║{reset}");
    println!("{cyan_b}   ║{reset}{}{cyan_b}║{reset}",
        center_pad(&format!("{dim}{tagline}{reset}"), BOX_INNER_W));
    println!("{cyan_b}   ╚═══════════════════════════════════════════════════════════════╝{reset}");
    println!();
    println!("{bold}   ┌─ context ─────────────────────────────────────────────────────┐{reset}");
    print_row(bold, dim, reset, "home    ", &truncate_to(&home_disp, BOX_VALUE_W));
    ...
    let model_value = if is_acp { "(remote agent loop)".into() } else { truncate_to(&cfg.model.default, BOX_VALUE_W) };
    ...
    let vault_value = if vault_count > 0 {
        format!("{green}{vault_count} entr{}{reset} {dim}· redactor active{reset}",
            if vault_count == 1 { "y" } else { "ies" })
    } else {
        format!("{dim}empty · pattern fallback only{reset}")
    };
    print_row(bold, dim, reset, "vault   ", &vault_value);
    print_row(bold, dim, reset, "skills  ", &format!("{skill_count} learned"));
    println!("{bold}   └───────────────────────────────────────────────────────────────┘{reset}");
    println!();
...
fn print_row(bold: &str, dim: &str, reset: &str, label: &str, value: &str) {
    ...
    println!(
        "{bold}   │{reset}  {dim}{label}:{reset} {value}{padding}{bold}│{reset}",
    );
}

fn center_pad(s: &str, width: usize) -> String {
    let visible = strip_ansi(s).chars().count();
    if visible >= width { return s.to_string(); }
...
fn use_color() -> bool {
    ...
    if std::env::var("EVO_NO_COLOR").is_ok() { return false; }
...
async fn count_vault_entries() -> usize {
    let path = match vault_path() { Ok(p) => p, Err(_) => return 0 };
...
async fn count_skills() -> Result<usize> {
    ...
    while let Some(e) = entries.next_entry().await? {
        if e.path().extension().and_then(|s| s.to_str()) == Some("yaml") { n += 1; }
    }
    Ok(n)
}
...
async fn handle_slash(rest: &str) -> Result<bool> {
    ...
    match cmd {
        "exit" | "quit" | "q"  => { println!("bye."); return Ok(true); }
        "help" | "?"           => print_help(),
        ...
        "clear"                => { print!("\x1b[2J\x1b[H"); std::io::stdout().flush().ok(); }
        "doctor"               => doctor().await?,
...
async fn run_provider_wizard() -> Result<()> {
    let choice = onboard::pick_provider().await?;
    if let Some(key) = onboard::ask_api_key(&choice).await? {
...
async fn doctor() -> Result<()> {
    ...
    let cfg = match load_config().await {
        Ok(c) => { println!("config   : OK ({})", config_path()?.display()); c }
        Err(e) => { println!("config   : MISSING — {e:#}\nrun `evoclaw onboard`"); return Ok(()); }
    };
    let provider_id = cfg.model.provider.clone().unwrap_or_else(|| "deepseek".into());
...
async fn run_one_shot(input: &str) -> Result<()> {
    ...
    let vault = Vault::load(&vault_path()?).await
        .wrap_err_with(|| format!("read vault at {}", vault_path().map(|p| p.display().to_string()).unwrap_or_default()))?;
    let redactor = Redactor::from_vault(&vault);
    if !redactor.is_empty() {
        println!("→ secret vault: {} entr{}",
            redactor.entry_count(),
            if redactor.entry_count() == 1 { "y" } else { "ies" });
    }
    let mut runtime = ConversationRuntime::new(
        provider, registry, session, tool_ctx,
        evo_core::runtime::RuntimeConfig {
            model: cfg.model.default.clone(), ..Default::default()
        },
    )
    ...
    println!("\n=== final ({} turns, {:.1}s) ===\n{}", outcome.turns, elapsed.as_secs_f32(), outcome.final_text);
    Ok(())
}
    ...
    for r in records {
        match r {
            ...
            evo_core::session::SessionRecord::Turn(t) => {
                ...
                if let Some(s) = &t.summary { println!("  summary: {s}"); }
                for tc in &t.tool_calls {
                    let preview = tc.result_truncated.lines().take(2).collect::<Vec<_>>().join(" | ");
                    let flag = if tc.is_error { "x" } else { "ok" };
                    println!("  [{flag}] {} args={} -> {}", tc.name, tc.args, preview);
                }
                if let Some(u) = &t.usage {
                    let hit = if u.input == 0 { 0.0 } else { u.cached as f64 / u.input as f64 * 100.0 };
                    println!("  usage: in={} cached={} ({:.0}% hit) out={}", u.input, u.cached, hit, u.output);
                }
                println!();
            }
...
async fn gateway(bind: &str, token: &str) -> Result<()> {
    ...
    let mut child = cmd.spawn()
        .map_err(|e| eyre::eyre!("evo-gateway binary not found on PATH: {e}. Build with `cargo build -p evo-gateway`."))?;
...
async fn doctor_closure() -> Result<()> {
    ...
    let mut entries = tokio::fs::read_dir(&dir).await?;
    let mut total = 0; let mut with_task = 0; let mut with_turns = 0;
    let mut with_end = 0; let mut completed = 0; let mut failed = 0;
    while let Some(entry) = entries.next_entry().await? {
        let p = entry.path();
        if p.extension().and_then(|s| s.to_str()) != Some("jsonl") { continue; }
        total += 1;
        let records = Session::read_all(&p).await.unwrap_or_default();
        let mut has_task = false; let mut has_turn = false; let mut end_state: Option<String> = None;
        for r in records {
            match r {
        ...
        if has_task { with_task += 1; }
        if has_turn { with_turns += 1; }
        if let Some(s) = &end_state {
            with_end += 1;
            if s.contains("COMPLETED") { completed += 1; }
            else if s.contains("FAILED") { failed += 1; }
        }
    }
...
async fn doctor_tokens() -> Result<()> {
    ...
    for ev in &events {
        if ev.ts >= day_cut {
            s7.0 += ev.input_tokens; s7.1 += ev.cached_tokens; s7.2 += ev.output_tokens;
            s7.3 += ev.usd; s7.4 += 1;
        }
        if ev.ts >= month_cut {
            s30.0 += ev.input_tokens; s30.1 += ev.cached_tokens; s30.2 += ev.output_tokens;
            s30.3 += ev.usd; s30.4 += 1;
        }
    }
    let hr = |c: u64, t: u64| -> f64 { if t == 0 { 0.0 } else { c as f64 / t as f64 } };
    ...
    println!("{:<14} {:>11.2}% {:>11.2}%", "cache_hit", hr(s7.1, s7.0)*100.0, hr(s30.1, s30.0)*100.0);
    println!("{:<14} {:>11.4}$ {:>11.4}$", "usd_total", s7.3, s30.3);
    println!();
    println!("budget: per_task ≤ ${:.2}, per_day ≤ ${:.2} (soft) / ${:.2} (hard), per_month ≤ ${:.0}",
        engine.cfg().per_task_usd, engine.cfg().per_day_soft_usd,
        engine.cfg().per_day_hard_usd, engine.cfg().per_month_usd);
    Ok(())
}
...
async fn skill_list() -> Result<()> {
    ...
    let mut entries = tokio::fs::read_dir(&dir).await?;
    println!("{:<24} {:<10} {:>5} {:<8} NAME", "ID", "STATE", "SCORE", "VER");
    while let Some(entry) = entries.next_entry().await? {
        let p = entry.path();
        if p.extension().and_then(|s| s.to_str()) != Some("yaml") { continue; }
        match Skill::load_yaml(&p).await {
            Ok(sk) => println!("{:<24} {:<10} {:>5.2} v{:<7} {}",
                sk.id, format!("{:?}", sk.state).to_uppercase(), sk.score, sk.version, sk.name),
...
async fn memory_search(query: &str, limit: usize) -> Result<()> {
    ...
    let hits = mem.search(query, &[MemoryLayer::L1, MemoryLayer::L2, MemoryLayer::L3], limit).await?;
    if hits.is_empty() {
...
async fn agent_list() -> Result<()> {
    let agents = evo_acp_client::list_agents().await
        .map_err(|e| eyre::eyre!("{e:#}"))?;
    if agents.is_empty() {
...
async fn agent_add(id: &str) -> Result<()> {
    let prof = evo_acp_client::find_agent(id).ok_or_else(|| eyre::eyre!(
        "unknown agent '{id}' — run `evoclaw agent catalog` to see options"))?;
    let cfg = evo_acp_client::AgentConfig::from_profile(prof);
    let path = evo_acp_client::save_agent(&cfg).await
        .map_err(|e| eyre::eyre!("{e:#}"))?;
...
async fn agent_remove(id: &str) -> Result<()> {
    evo_acp_client::remove_agent(id).await.map_err(|e| eyre::eyre!("{e:#}"))?;
    println!("removed agent '{id}'");
    Ok(())
}

async fn agent_test(id: &str) -> Result<()> {
    let cfg = evo_acp_client::load_agent(id).await
    ...
    client.spawn(&cfg).await.map_err(|e| eyre::eyre!(
        "{e}; install with: {}", evo_acp_client::find_agent(id).map(|p| p.install_hint).unwrap_or("see catalog")))?;
    let result = client.initialize("evoclaw", env!("CARGO_PKG_VERSION")).await
...
async fn mcp_list() -> Result<()> {
    let servers = evo_mcp_client::list_servers().await
        .map_err(|e| eyre::eyre!("{e:#}"))?;
    if servers.is_empty() {
...
async fn mcp_add(id: &str) -> Result<()> {
    let prof = evo_mcp_client::find_server(id).ok_or_else(|| eyre::eyre!(
        "unknown server '{id}' — run `evoclaw mcp catalog`"))?;
    ...
    let path = evo_mcp_client::save_server(&cfg).await
        .map_err(|e| eyre::eyre!("{e:#}"))?;
    println!("✓ added MCP server '{id}' → {}", path.display());
    println!("  install: {}", prof.install_hint);
    if !prof.auth_env.is_empty() {
        let captured: Vec<&String> = cfg.env.iter().map(|(k,_)| k).collect();
        let missing: Vec<&&str> = prof.auth_env.iter()
            .filter(|v| !captured.iter().any(|c| c.as_str() == **v)).collect();
        if !missing.is_empty() {
...
async fn mcp_remove(id: &str) -> Result<()> {
    evo_mcp_client::remove_server(id).await.map_err(|e| eyre::eyre!("{e:#}"))?;
    println!("removed MCP server '{id}'");
    Ok(())
}

async fn mcp_test(id: &str) -> Result<()> {
    let cfg = evo_mcp_client::load_server(id).await
    ...
    let _ = client.initialize("evoclaw", env!("CARGO_PKG_VERSION")).await
        .map_err(|e| eyre::eyre!("initialize failed: {e}"))?;
    println!("✓ initialize OK");
...
async fn read_secret_from_stdin() -> Result<String> {
    ...
    if v.is_empty() { return Err(eyre::eyre!("empty value — aborted")); }
    Ok(v)
}
...
async fn secret_add(name: &str, from_stdin: bool, value: Option<String>) -> Result<()> {
    ...
    let entry = vault.get(name).ok_or_else(|| eyre::eyre!("upsert vanished"))?;
    println!("✓ stored '{name}' (kind={}, fingerprint={}) at {}",
        entry.kind, entry.fingerprint, path.display());
...
async fn secret_list() -> Result<()> {
    ...
    for e in vault.list() {
        println!("{:<24} {:<14} {:<10} {}",
            e.name, e.kind, e.fingerprint, e.created_at.format("%Y-%m-%d %H:%M"));
    }
    println!("\n(values are stored at {} — chmod 600)", vault_path()?.display());
    Ok(())
}
...
async fn most_recent_session() -> Result<PathBuf> {
    ...
    newest.map(|(_, p)| p).ok_or_else(|| eyre::eyre!("no JSONL sessions in {}", dir.display()))
}
